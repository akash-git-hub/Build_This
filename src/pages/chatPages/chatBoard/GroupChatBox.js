...
export const GroupChatBox = ({ chat, handleSend, messages, currentUser }) => {
    ...
    return (
        <Stack direction='vertical' gap={1} >
            <Stack direction='horizontal' gap={3} className='mx-3'>
                <Image
                    src={chat?.groupImage ? chat?.groupImage : '/assets/images/user.svg'}
                    style={{ maxWidth: "50px" }}
                    fluid rounded />
                <Stack direction='vertical' gap={0}>
                    <h5 className='mb-0'>{chat?.groupName}</h5>
                            ...
                            return (
                                (msg?.senderId === currentUser?.id) ? (
                                    <Stack direction="horizontal" gap={2} style={{ justifyContent: 'end', alignItems: 'end' }}>
                                        ...
                                        <Image src={msg.image ? msg.image : '/assets/images/user.svg'} roundedCircle className="mb-4" style={{ maxWidth: '30px' }} />
